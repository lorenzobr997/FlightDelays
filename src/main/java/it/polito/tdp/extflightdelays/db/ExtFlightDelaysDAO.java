...
public class ExtFlightDelaysDAO {
	...
	public List<Airport> getVertici(int x, Map<Integer, Airport> idMap){
		...
		List<Airport> res = new LinkedList<>();
		try {
			Connection conn = ConnectDB.getConnection();
			PreparedStatement st = conn.prepareStatement(sql);
			st.setInt(1, x);
			...
			while (rs.next()) {
				res.add(idMap.get(rs.getInt("id_vertice")));
			}
			rs.close();
			st.close();
			conn.close();
	...
	public List<Rotta> getRotte(Map<Integer, Airport> idMap){
		...
		List<Rotta> res = new LinkedList<Rotta>();
		try {
			Connection conn = ConnectDB.getConnection();
			PreparedStatement st = conn.prepareStatement(sql);
			ResultSet rs = st.executeQuery();
			...
			while (rs.next()) {
				Airport source = idMap.get(rs.getInt("id_a1"));
				Airport dest = idMap.get(rs.getInt("id_a2"));
				if(source != null && dest != null)
					res.add(new Rotta(source, dest, rs.getInt("n")));
				else {
